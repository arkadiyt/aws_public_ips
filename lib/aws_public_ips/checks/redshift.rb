  ...
  module Checks
    module Redshift
      def self.run(options)
        ...
        ::AwsPublicIps::Utils.probe(::Aws::Redshift::Client, options[:regions], options[:progress]) do |client|
          client.describe_clusters.flat_map do |response|
            response.clusters.flat_map do |cluster|
              ...
              if cluster.endpoint.nil?
                raise StandardError, "Redshift cluster '#{cluster.cluster_identifier}' has a nil endpoint. This" \
                  ' likely means the cluster is being brought up right now.'
              end
